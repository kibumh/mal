		...
		case MalList:
			...
			if op, ok := v.List[0].(MalSymbol); ok {
				...
				case MalSymbol("quasiquote"):
					...
					continue // TCO
				case MalSymbol("try*"):
					evaled, err := EVAL(v.List[1], env)
					if err != nil {
						if len(v.List) != 3 { // No (catch ...)
							return nil, err
						}
						newEnv := NewEnv(env, NewMalList(), nil)
						catchExp := v.List[2].(MalList).List
						if exc, ok := err.(MalException); ok {
							newEnv.Set(catchExp[1].(MalSymbol), exc.Value)
						} else {
							newEnv.Set(catchExp[1].(MalSymbol), MalString(err.Error()))
						}
						return EVAL(catchExp[2], newEnv)
					}
					return evaled, nil
				}
			}
...
func rep(line string, env *Env) (ret string) {
	mv, err := READ(line)
	...
	if err != nil {
		if exc, ok := err.(MalException); ok {
			return "MalException: " + PRINT(exc.Value)
		} else {
			return "MalException: " + err.Error()
		}
	}
	return PRINT(mv)
}
...
func initEnv() *Env {
	...
	env.Set(MalSymbol("eval"), MalFunc{
		Body: func(ps ...MalType) (MalType, error) {
	...
	rep(`(def! *host-language* "go.2")`, env)
